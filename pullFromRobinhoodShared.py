...
put_options_data = {day: {} for day in fridays}
put_options_data_short = {}
...
for ticker in lines:
    try:
        ...
        # Fetch the latest stock price for the ticker
            ...
            if maxPrice>float(stock_price[ticker][0])>minPrice:
                for day in fridays:
                    ...
                    # Fetch call options data for the ticker and expiration day
                    put_options = []
                    put_options = robin.find_options_by_expiration(ticker, str(day),optionType='put')
                    # Store call options data in the call_options_data dictionary
                    #
                    
                    #Now to the puts
                    put_options_data[day][ticker] = put_options
                    strikePrice = []
                    bidPrice = []
                    diff = []
                    ...
                    # Create new lists containing only the options with positive differences
                    put_options_filtered = [put_options[i] for i in range(len(put_options)) if i not in rm_row]
                    diff_filtered = [diff[i] for i in range(len(diff)) if i not in rm_row]
                    
                    # Update put_options and diff with the filtered lists
                    put_options = put_options_filtered
                    diff = diff_filtered

                    # Get the sorted indices and values simultaneously
                    sorted_indices_and_values = sorted(enumerate(diff), key=lambda x: x[1],reverse=True)
                    ...
                    tempOption = []
                    if sorted_indices:  # Check if sorted_indices is not empty
                        tempOption = put_options[sorted_indices[0]]
                        outFill = [
                            ticker,
                            float(stock_price[ticker][0]),  # Convert to float
                            float(tempOption['strike_price']),
                        ...
                        # Append outFill to the list corresponding to the day
                        if day_str not in put_options_data_short:
                            put_options_data_short[day_str] = []  # Initialize list if it doesn't exist
                        put_options_data_short[day_str].append(outFill)
...
for day in call_options_data_short:
    ...
    # Append outFill to the list corresponding to the day
    if day_str not in out_sorted_calls:
        out_sorted_calls[day_str] = []  # Initialize list if it doesn't exist
        out_sorted_puts[day_str] = []  # Initialize list if it doesn't exist
    out_sorted_calls[day_str] = sorted(call_options_data_short[day_str], key=lambda x: x[4],reverse=True)  # Sort in descending order
    out_sorted_puts[day_str] = sorted(put_options_data_short[day_str], key=lambda x: x[4],reverse=True)  # Sort in descending order
